...
kf.F (Durum Geçiş Matrisi): Sistemin bir zaman adımından diğerine nasıl geçtiğini modeller. Benim kullandığım "sabit hız" modeline göre, yeni konum = eski konum + (hız * zaman_aralığı) ve yeni hız = eski hız. Bu matris, ana döngü içinde geçen süreye bağlı olarak sürekli güncellenir. Kodumda kf.R *= 0.3 satırı ile varsayılan gürültü değerini biraz düşürerek GPS ölçümlerine olan güvenimi bir miktar artırdım. Bu değeri, gerçek dünya testlerinde aracın Kalman filtresiyle elde edilen konumunun ham GPS verisine ne kadar yakın olmasını istediğime göre ayarladım.
...
kf.Q (Süreç Gürültüsü Kovaryansı): Bu matris ise, kullandığım hareket modelinin (sabit hız varsayımı) gerçek dünyaya ne kadar uyduğunu temsil eder. Q matrisindeki değerler ne kadar büyükse, filtre kendi hareket modeline o kadar az güvenir. Bir yarış arabası sürekli hızlanıp yavaşladığı için "sabit hız" modeli mükemmel değildir. Q matrisi, bu modeldeki belirsizliği (hızlanma gibi) temsil eder. Değerleri artırmak, filtrenin ani yön ve hız değişikliklerine daha hızlı tepki vermesini sağlar, ancak bu durum filtrelenmiş veriyi daha "titrek" hale getirebilir. Bu iki parametre (R ve Q) arasındaki dengeyi bulmak, Kalman Filtresi ayarının en kritik ve tecrübe gerektiren kısmıdır ve "filtre ayarı" (filter tuning) olarak adlandırılır. Yaptığım testler sonucunda kodda görülen değerlerin, pürüzsüz bir takip ile ani manevralara hızlı yanıt verme arasında iyi bir denge kurduğunu gözlemledim.
